"""
===============================================================================
ex23 — Galaxy lens with external convergence + shear (SLACS-style)
===============================================================================
Composite scenario: a SLACS-like early-type galaxy (PEMD with gamma~2.05 and
q_ax=0.7) embedded in the tidal field of a nearby cluster (kappa_ext = 0.04,
gamma_+_ext = 0.06). External shear breaks the residual axisymmetry left by
the elliptical galaxy potential alone, splitting near-Einstein rings into
quadruply-imaged systems with twisted asymmetry — the hallmark of
group/cluster-perturbed galaxy lenses (e.g. SDSS J0946+1006 "Jackpot").

Same JWST-style pipeline as ex19/ex20/ex21/ex22.

Run from repo root:
    MPLCONFIGDIR=/tmp/mplconfig python ex23.galaxy_with_shear.py
===============================================================================
"""
from math import pi
...
def main():
    ...
    q_ax = 0.70
    K = 1.0e-5
    base = pemd.LensMetric(K=K, gamma=gamma, q_ax=q_ax, r_min=1e-1)

    # First estimate b_ring from the spherical PEMD limit (no external
    # contribution yet) so we can size the shear localization window.
    target = scene.D_S / (scene.D_L * scene.D_LS)
    def lens_eq(b):
        return base.deflection_angle_spherical(b) - b * target
    b_ring = brentq(lens_eq, 1e2, 1e9)

    kappa_ext = 0.04
    gamma_plus_ext = 0.06
    gamma_cross_ext = 0.0
    # Window the external potential to ~5 b_ring so it acts mostly
    # within the lensing region but is suppressed at very large r.
    r_window = 5.0 * b_ring
    lens = composite.with_external(
        base, kappa=kappa_ext,
        gamma_plus=gamma_plus_ext,
        gamma_cross=gamma_cross_ext,
        r_window=r_window)
    print(f"Base PEMD: K={K:.2e}, gamma={gamma}, q_ax={q_ax}")
    print(f"External: kappa={kappa_ext}, gamma_+={gamma_plus_ext}, "
          f"gamma_x={gamma_cross_ext}")
    print(f"b_ring (spherical estimate, kappa-corrected): {b_ring:.3e}")

    detector = image_plane.detector(
        D=scene.D_L, iota=pi/2, x_pixels=256,
    ...
    np.savez(datadir / "galaxy_with_shear.npz",
             rgb=rgb_disp, blue=img_blue, red=img_red,
             galaxy=img_gal, stars=stars,
             K=K, gamma=gamma, q_ax=q_ax, b_ring=b_ring,
             kappa_ext=kappa_ext, gamma_plus_ext=gamma_plus_ext,
             gamma_cross_ext=gamma_cross_ext,
             z_L=scene.z_lens, z_S=scene.z_source)

    fig, ax = plt.subplots(1, 1, figsize=(7, 7), constrained_layout=True)
    ax.imshow(np.transpose(rgb_disp, (1, 0, 2)), origin="lower")
    ax.set_title(
        f"PEMD+shear (gamma={gamma}, q_ax={q_ax}, kappa={kappa_ext}, gamma_+={gamma_plus_ext})\n"
        f"z_L={scene.z_lens}, z_S={scene.z_source}, "
        f"M_lens={M_lens_Msun:.0e} Msun")
    ax.set_xticks([]); ax.set_yticks([])
    output = outdir / "galaxy_with_shear.png"
