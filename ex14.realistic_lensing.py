"""
===============================================================================
Realistic Strong Gravitational Lensing with Multiple Sources and Asymmetry
===============================================================================
Pipeline:
    1. SIE galaxy lens (elliptical, not perfectly symmetric)
    2. Multiple lensed background sources (one dominant + several companion galaxies)
    3. Multiple background galaxies projected directly (not lensed)
    4. Starfield with realistic density
    5. RGB composition (lens = orange, arcs = blue, stars/galaxies = white/red)
    6. Realistic photographic noise (Poisson + read noise + Gaussian)
    7. asinh stretch -> PNG + npy

Result: Einstein ring that is incomplete, with multiple light sources and
realistic irregularities — similar to actual HST observations.
===============================================================================
"""
...
# Approximate Einstein radius for SIE (similar to SIS but slightly different)
b_ring = 4.0 * pi * sigma_v**2 * D_LS     # ~107 M
x_side = 3.2 * b_ring                     # larger FOV to see background galaxies
x_pixels = 1920
...
===============================================================================
'''
##### PRIMARY SOURCE: Large offset + smaller size -> incomplete ring ("horseshoe")
primary_source = Sersic(x0=0.12*b_ring, y0=0.04*b_ring,
                        R_e=0.08*b_ring, n=1.0, I_e=1.0,
                        ell=0.20, pa=pi/4)

##### SECONDARY SOURCES: Small companion galaxies (magnified by lensing)
# These add complexity and break perfect symmetry
secondary_sources = [
    Sersic(x0=0.15*b_ring, y0=-0.08*b_ring,
           R_e=0.04*b_ring, n=1.5, I_e=0.5, ell=0.10, pa=-pi/3),
    Sersic(x0=0.08*b_ring, y0=0.12*b_ring,
           R_e=0.035*b_ring, n=1.0, I_e=0.4, ell=0.05, pa=0.0),
]
...
===============================================================================
'''
##### Multiple faint background galaxies (projected directly, not lensed)
##### These simulate the deep-field environment and add realism
background_galaxies = [
...
===============================================================================
'''
##### Dense starfield (more realistic for HST-like observations)
n_stars = 150
...
===============================================================================
'''
filename = 'realistic_lensing_sie'
savefig = True
...
for secondary_arc in secondary_arcs:
    arc_img = np.maximum(arc_img, secondary_arc)  # composite (brightest pixel)
...
for bg_gal in background_galaxies:
    ...
    gal_img = np.maximum(gal_img, gal_projected)  # composite
if gal_img.max() > 0:
    gal_img /= gal_img.max()
...
print("Composing RGB...")
# Blend lens, arcs, background galaxies, and stars
